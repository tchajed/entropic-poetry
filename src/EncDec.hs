...
type DbMonad a = forall k. Monad k =>
                             ReaderT WordList k a

type CtxMT m a = ReaderT WordList (StateT Ctx m) a

type CtxM a = forall m. Monad m =>
                          CtxMT m a
...
lookupName name@(Name n) =
  let def = "{?" ++ n ++ "}"
  in gets (Map.findWithDefault def name)
...
encode wl fmt d = runCtxM encodeM wl
  where
    encodeM :: CtxM (String, [VB.Word8])
    encodeM = do
      cs <- cardinalities fmt
      let (ws, d') = VB.encode cs (BS.unpack d)
        in do encoded <- encodeWords fmt (ws ++ [0,0 ..])
              return (encoded, d')
...
decodeType t s = do
  ...
  let parse i w = do
      ...
      possibleParses = catMaybes $ zipWith parse [0 ..] ws
    in liftPossibilities possibleParses
  ...
  in case parses of
       ...
       _ ->
         case completeParses of
           [] -> Left NoParse
           [p] ->
             let numBytes = floor (entropyBytes wl fmt)
             in Right (BS.take numBytes p)
