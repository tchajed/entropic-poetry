...
parseCommand =
  let parseInput = inputFromStr <$> argument str (metavar "input")
  in subparser
       (command
          "enc"
          (info (EncCommand <$> parseInput) (progDesc "encode data into poetry")) <>
        command
          "dec"
          (info (DecCommand <$> parseInput) (progDesc "decode poem into data")))

parseOpts :: Parser PoetryOptions
parseOpts =
  PoetryOptions <$>
  strOption
    (long "format" <> short 'f' <> metavar "FILENAME" <>
     help "filename for format schema") <*>
  strOption
    (long "words" <> short 'l' <> metavar "WORDLIST" <> showDefault <>
     value "wordlist.txt" <>
     help "categorized word list") <*>
  (outputFromStr <$>
   strOption
     (long "output" <> short 'o' <> metavar "FILENAME" <> showDefault <>
      value "-" <>
...
readFormat fname = do
  fmt <- withFile fname ReadMode $ \h -> parseFormat fname <$> hGetContents h
  case fmt of
    Left e -> do
...
readWordList fname = do
  wl <- withFile fname ReadMode $ \h -> parseWordList fname <$> hGetContents h
  case wl of
    Left e -> do
...
main = execParser opts >>= mainOp
  where
    opts =
      info
        (parseOpts <**> helper)
        (fullDesc <> progDesc "Encode and decode using a poem format" <>
